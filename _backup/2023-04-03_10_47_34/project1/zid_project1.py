...
def get_tics(pth):
    ...
    """
    with open(pth) as tics:
        tic = {}
        for i, line in enumerate(tics):
...
def read_dat(tic):
    ...
    tic = os.path.join(DATDIR, f'{tic}_prc.dat')
    with open(tic) as f:
        tic_list = []
        for i, line in enumerate(f):
            line = line.lower().strip()
            if line:
                tic_list.append(line)
            else:
                break
        return tic_list
...
def line_to_dict(line):
    ...
    if line:
        ...
        for key, value in COLWIDTHS.items():
            ...
            tic_dict[key] = line[start:end].strip()
            start = end
    return tic_dict
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    tickers_lst = []
    empty_lst = []
    for key, value in tic_exchange_dic.items():
        tickers_lst.append(key)
        print(tickers_lst)
        if tickers_lst == empty_lst:
            raise Exception
        elif tickers_lst != tic_exchange_dic.keys():
            raise Exception
        elif tickers_lst == tic_exchange_dic.keys():
            return None
...
def verify_cols(col_lst=None):
    ...
    """
    col_lst = ['Volume', 'Date', 'Adj Close', 'Close', 'Open', 'High']
    empty_list = []
    if col_lst == empty_list:
        raise Exception
    elif col_lst != COLUMNS:
        raise Exception
    elif col_lst == COLUMNS:
        return None
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    """
    tickers_lst = tickers_lst[0:]
    col_lst = col_lst[0:]
    data_dict = {}
    for ticker, exchange in tic_exchange_dic.items():
        if ticker in tickers_lst:
            line = read_dat(ticker)
            data_list = []
            for data_line in line:
                data = line_to_dict(data_line)
                data = {k: v for k, v in data.items() if k in col_lst}
...
def create_json(data_dict, pth):
    ...
    data_dict = create_data_dict(tic_exchange_dic, tickers_lst=['aapl'], col_lst=['Date', 'Open', 'Close'])
    save = open('data_dict', 'w')
    json.dump(data_dict, save)
    return
