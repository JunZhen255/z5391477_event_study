...
def read_prc_csv(tic):
    ...
    """
    path_ = os.path.join(cfg.DATADIR, tic.lower() + '_prc.csv')
    df = pd.read_csv(path_, parse_dates=['Date'])
...
def mk_ret_df(prc_df):
    ...
    """
    df = pd.read_csv(cfg.FF_CSV, parse_dates=['Date'], index_col=['Date'])
    df = df['mkt']
    prc_df = prc_df.pct_change()
    prc_df = prc_df.join(df, how='right')
    return prc_df
